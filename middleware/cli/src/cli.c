...
static AT_status_t _CLI_ths_callback(void) {
    // Local variables.
    AT_status_t status = AT_SUCCESS;
    AHT2X_status_t sht3x_status = AHT2X_SUCCESS;
    ...
    sht3x_status = AHT2X_get_temperature_humidity(I2C_ADDRESS_AHT20, &temperature_tenth_degrees, &humidity_percent);
    _CLI_check_driver_status(sht3x_status, AHT2X_SUCCESS, ERROR_BASE_AHT20);
    // Read and print data.
    // Temperature.
...
static AT_status_t _CLI_acc_callback(void) {
    ...
    POWER_enable(POWER_REQUESTER_ID_CLI, POWER_DOMAIN_SENSORS, LPTIM_DELAY_MODE_SLEEP);
    // Get chip ID.
    LIS2DH12_status_t mma865xfc_status = LIS2DH12_get_id(I2C_ADDRESS_LIS2DH12, &chip_id);
    _CLI_check_driver_status(mma865xfc_status, LIS2DH12_SUCCESS, ERROR_BASE_LIS2DH12);
    // Print data.
